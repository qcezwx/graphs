...
def optgraph_brute(graph_eges, vertex_score, limit):
    ...
    while i.not_is_over():
        ...
        for node in node_list:  # can be raplaced with "in range"
            if i.is_in_list(
                    node):  # вершина должна быть интом, иначе смотри комментарии итератора
                node_sublist.append(node)
        score = get_score(node_sublist,
                          vertex_score)  # считать с предыдущим шагом -- минус один проход
        if score > max_score:
...
def optgraph_spath_greedy(graph_eges, vertex_score, limit):
    ...
    path_weight = dict(
        nx.all_pairs_dijkstra_path_length(G, cutoff=None, weight='weight'))
    ...
    for i in node_list:
        ...
        for j in node_list:
            if i != j:
                path_rate[i][j] = get_path_rate_greedy(path[i][j], path_weight[i][j],
                                                       vertex_score)
            else:
                path_rate[i][j] = 0
    ...
    while 1:
        ...
        for n in vacant_nodes:
            d, weight = get_min_path(n, selected_nodes,
                                     path_weight)  # для каждой вершины вакантной берем ближайшую из уже выбранных и длинну
            #            if(weight > weight_left):
            #                unreachable.append(n) #если вершина слишком далеко -- удаляем
            if (path_rate[d][n] > rate) and (
                    weight <= weight_left):  # выбираем путь с наибольшим рейтингом
                dest = d
                rate = path_rate[n][d]
                node = n
...
def optgraph_simple_greedy(graph_eges, vertex_score, limit):
    ...
    while len(vacant_nodes) > 0:
        best_node = next(iter(
            vacant_nodes))  # костыль, но нет способа красиво взять один элемент из сета
...
def optgraph_spath_grasp(graph_eges, vertex_score, limit, rep=10, alpha=0.7):
    ...
    for i in range(rep):
        ...
        for i in node_list:
            ...
            for j in node_list:
                if i != j:
                    path_rate[i][j] = get_path_rate_greedy(path[i][j],
                                                           path_weight[i][j],
                                                           vertex_score)
                else:
                    path_rate[i][j] = 0
        ...
        while 1:
            ...
            selected_path.remove(
                dest)  # костыль. научиться добавлять не создавая копию
            # добавить в выбранные и удалить из вакантных все вершины пути
            selected_nodes.extend(selected_path)
            for i in selected_path:
                vacant_nodes.remove(i)
